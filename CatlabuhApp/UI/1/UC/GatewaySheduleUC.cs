...
namespace CatlabuhApp.UI.UC
{
    public partial class GatewaySheduleUC : UserControl, IDataViewUC
    {
        ...
        public string[] GatewayOpenVD
        {
            get
            {
                return GetGatewayWorkDates(VDGDPlusBoxes);
            }
        }
        public string[] GatewayCloseVD
        {
            get
            {
                return GetGatewayWorkDates(VDGDMinusBoxes);
            }
        }
        public string[] GatewayOpenVoz
        {
            get
            {
                return GetGatewayWorkDates(VozGDPlusBoxes);
            }
        }
        public string[] GatewayCloseVoz
        {
            get
            {
                return GetGatewayWorkDates(VozGDMinusBoxes);
            }
        }

        public bool IsCalculateGS => calculate.Checked;
        ...
        public void ClearFields()
        {
            for (int i = 0; i < 12; i++)
            {
                VDGDPlusBoxes[i].Clear();
                VDGDMinusBoxes[i].Clear();
                VozGDPlusBoxes[i].Clear();
                VozGDMinusBoxes[i].Clear();
                VD_plusBoxes[i].Clear();
                VD_minusBoxes[i].Clear();
                Voz_plusBoxes[i].Clear();
                Voz_minusBoxes[i].Clear();
            }
        }

        private string[] GetGatewayWorkDates(List<TextBox> someBoxes)
        {
            string[] output = new string[someBoxes.Count];

            for (int i = 0; i < someBoxes.Count; i++)
            {
                output[i] = someBoxes[i].Text;
            }
        ...
        public void LoadShedule()
        {
            ...
            {
                DataAccess.GetColumnData<string>($"SELECT GatewayOpenVD FROM GatewaySchedule WHERE YearName = {YearOfCalculation}").ToArray(),
                DataAccess.GetColumnData<string>($"SELECT GatewayCloseVD FROM GatewaySchedule WHERE YearName = {YearOfCalculation}").ToArray(),
                DataAccess.GetColumnData<string>($"SELECT GatewayOpenVoz FROM GatewaySchedule WHERE YearName = {YearOfCalculation}").ToArray(),
                DataAccess.GetColumnData<string>($"SELECT GatewayCloseVoz FROM GatewaySchedule WHERE YearName = {YearOfCalculation}").ToArray()
            };

            for (int i = 0; i < 12; i++)
            {
                // 24 length
                VDGDPlusBoxes[i].Text = dates[0][i];
                VDGDMinusBoxes[i].Text = dates[1][i];
                VozGDPlusBoxes[i].Text = dates[2][i];
                VozGDMinusBoxes[i].Text = dates[3][i];
        ...
        private void ContextMenuItem_Click(object sender, EventArgs e)
        {
            switch (((ToolStripMenuItem)sender).Name)
            {
                case "calculate":
                    ...
                    enterVolumes.Checked = false;
                    break;
                case "enterVolumes":
                    calculate.Checked = false;
                    enterVolumes.Checked = true;
                    break;
            }
        }
